...
CREATE POLICY "Users can manage package evidence for their firm" ON public.package_evidence
FOR ALL USING (
  package_id IN (
    SELECT fp.id FROM public.form_packages fp
    JOIN public.cases c ON c.id = fp.case_id
